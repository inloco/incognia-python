...
class TestBaseRequest(TestCase):
    ...
    @patch('requests.post')
    def test_post_when_parameters_are_valid_should_return_a_valid_dict(self, mock_requests_post: Mock):
        def get_mocked_response() -> requests.Response:
            response = requests.Response()
            response._content, response.status_code = encode(self.JSON_RESPONSE), self.OK_STATUS_CODE
            return response
        ...
        mock_requests_post.assert_called_with(url=self.URL, headers=USER_AGENT_HEADER, data=None, params=None,
                                              timeout=base_request.timeout(), auth=None)
        self.assertEqual(result, self.JSON_RESPONSE)

    @patch('requests.post')
    def test_post_when_parameters_are_invalid_should_raise_an_IncogniaHTTPError(self, mock_requests_post: Mock):
        def get_mocked_response() -> requests.Response:
            response = requests.Response()
            response._content, response.status_code = encode(self.JSON_RESPONSE), self.CLIENT_ERROR_CODE
            return response
    ...
    @patch('requests.get')
    def test_get_when_parameters_are_valid_should_return_a_valid_dict(self, mock_requests_get: Mock):
        def get_mocked_response() -> requests.Response:
            response = requests.Response()
            response._content, response.status_code = encode(self.JSON_RESPONSE), self.OK_STATUS_CODE
            return response
    ...
    @patch('requests.get')
    def test_get_when_parameters_are_invalid_should_raise_an_IncogniaHTTPError(self, mock_requests_get: Mock):
        def get_mocked_response() -> requests.Response:
            response = requests.Response()
            response._content, response.status_code = encode(self.JSON_RESPONSE), self.CLIENT_ERROR_CODE
            return response

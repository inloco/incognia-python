...
class TestTokenManager(TestCase):
    ...
    JSON_POST_RESPONSE: Final[ascii] = b'{ "access_token": "ACCESS_TOKEN", "token_type": "TOKEN_TYPE",' \
                                       b' "expires_in": 900 }'
    SHORT_EXPIRATION_TIME_TOKEN_VALUES: Final[TokenValues] = TokenValues('S_E_T_ACCESS_TOKEN', 'S_E_T_TOKEN_TYPE')
    SHORT_EXPIRATION_TIME_JSON_POST_RESPONSE: Final[ascii] = b'{ "access_token": "S_E_T_ACCESS_TOKEN",' \
                                                             b' "token_type": "S_E_T_TOKEN_TYPE", "expires_in": 5 }'
    BASE64URL: Final[str] = base64.urlsafe_b64encode(f'{CLIENT_ID}:{CLIENT_SECRET}'.encode('ascii')).decode('utf-8')
    HEADERS: Final[dict] = {'Authorization': f'Basic {BASE64URL}'}
    OK_STATUS_CODE: Final[int] = 200
    CLIENT_ERROR_CODE: Final[int] = 400

    @patch('requests.post')
    def test_get_when_credentials_are_valid_should_return_a_valid_token(self, mock_requests_post: Mock):
    ...
    @patch('requests.post')
    def test_get_when_credentials_are_invalid_should_raise_an_IncogniaHTTPError(self, mock_requests_post: Mock):
        def get_mocked_response() -> requests.Response:
            response = requests.Response()
    ...
    @patch('requests.post')
    def test_get_when_token_is_expired_should_return_a_new_valid_token(self, mock_requests_post: Mock):
        token_manager = TokenManager(self.CLIENT_ID, self.CLIENT_SECRET)

        def get_first_mocked_response() -> requests.Response:
            response = requests.Response()
            response._content, response.status_code = self.SHORT_EXPIRATION_TIME_JSON_POST_RESPONSE, self.OK_STATUS_CODE
            return response
        ...
        self.assertEqual(first_token_values, self.SHORT_EXPIRATION_TIME_TOKEN_VALUES)
